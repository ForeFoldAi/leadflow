...
class TwoFactorAuthService {
    constructor() {
        ...
        this.MAX_ATTEMPTS = 3; // Maximum OTP attempts
    }
    ...
    async sendTwoFactorOTP(userId, userEmail, userName) {
        try {
    ...
    // Verify 2FA OTP
    verifyTwoFactorOTP(userId, providedOTP) {
        try {
            // Clean up expired sessions first
            this.cleanupExpiredSessions();
            const sessionKey = this.createSessionKey(userId);
    ...
    // Verify backup code
    verifyBackupCode(userId, providedCode, storedCodes) {
