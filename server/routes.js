...
export async function registerRoutes(app) {
    ...
    app.get("/api/leads/export", async (req, res) => {
        try {
            ...
            const csvRows = leads.map(lead => [
                ...
                `"${lead.leadSource ? (lead.leadSource === 'other' && lead.customLeadSource
                ...
                try {
                    ...
                    try {
                        ...
                        if (activeUsers.length > 0 && createdLeads.length > 0) {
                            await notificationService.notifyBatchImport(activeUsers.map(user => ({ id: user.id, email: user.email })), createdLeads.length, req.user?.email || 'Unknown');
                        }
                    }
                    catch (notificationError) {
    ...
    app.put("/api/users/profile", async (req, res) => {
        try {
            ...
            if (updates.role)
                fieldsToUpdate.role = updates.role;
            if (updates.customRole)
                fieldsToUpdate.customRole = updates.customRole;
            if (updates.companyName && updates.companyName.trim() !== '')
                fieldsToUpdate.companyName = updates.companyName;
            if (updates.companySize)
                fieldsToUpdate.companySize = updates.companySize;
            if (updates.industry && updates.industry.trim() !== '')
                fieldsToUpdate.industry = updates.industry;
            if (updates.website)
                fieldsToUpdate.website = updates.website;
            if (updates.phoneNumber)
                fieldsToUpdate.phoneNumber = updates.phoneNumber;
            if (updates.password)
                fieldsToUpdate.password = updates.password;
    ...
    app.get("/api/analytics", authenticateUser, async (req, res) => {
        try {
            ...
            const convertedLeadsWithDates = leads.filter(lead => lead.leadStatus === "converted" &&
                lead.lastContactedDate &&
                lead.nextFollowupDate);
