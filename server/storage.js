...
import { leads, users, userPreferences, notificationSettings, securitySettings, notificationLogs, userSessions, passwordResets } from "@shared/schema";
...
export class SqlStorage {
    ...
    async createLead(insertLead, userId) {
        try {
            const result = await this.db
                .insert(leads)
                .values({
    ...
    async batchCreateLeads(insertLeads, userId) {
        try {
    ...
    async searchLeads(query, userId) {
        try {
            ...
            if (userId) {
                return await this.db
                    .select()
                    .from(leads)
                    .where(and(eq(leads.userId, userId), or(ilike(leads.name, searchTerm), ilike(leads.email, searchTerm), ilike(leads.phoneNumber, searchTerm), ilike(leads.city, searchTerm), ilike(leads.state, searchTerm), ilike(leads.country, searchTerm), ilike(leads.pincode, searchTerm), ilike(leads.companyName, searchTerm), ilike(leads.designation, searchTerm), ilike(leads.lastContactedBy, searchTerm), ilike(leads.customerInterestedIn, searchTerm), ilike(leads.additionalNotes, searchTerm), ilike(leads.leadStatus, searchTerm), ilike(leads.customerCategory, searchTerm), ilike(leads.preferredCommunicationChannel, searchTerm), ilike(leads.leadSource, searchTerm))))
                    .orderBy(desc(leads.createdAt));
            }
            else {
                return await this.db
                    .select()
                    .from(leads)
                    .where(or(ilike(leads.name, searchTerm), ilike(leads.email, searchTerm), ilike(leads.phoneNumber, searchTerm), ilike(leads.city, searchTerm), ilike(leads.state, searchTerm), ilike(leads.country, searchTerm), ilike(leads.pincode, searchTerm), ilike(leads.companyName, searchTerm), ilike(leads.designation, searchTerm), ilike(leads.lastContactedBy, searchTerm), ilike(leads.customerInterestedIn, searchTerm), ilike(leads.additionalNotes, searchTerm), ilike(leads.leadStatus, searchTerm), ilike(leads.customerCategory, searchTerm), ilike(leads.preferredCommunicationChannel, searchTerm), ilike(leads.leadSource, searchTerm)))
                    .orderBy(desc(leads.createdAt));
            }
        }
        catch (error) {
    ...
    async filterLeads(filters, userId) {
        try {
            const conditions = [];
    ...
    async getPasswordResetByOtp(email, otp) {
        try {
            const result = await this.db
                .select()
                .from(passwordResets)
